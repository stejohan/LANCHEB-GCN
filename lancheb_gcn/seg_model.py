...
class base_model(object):
    ...
    def evaluate(self, data, cat, labels, sess=None):
        ...
        return string, accuracy, f1, loss, predictions # Return predictions for plotting test data
    ...
    def fit(self, train_data, train_cat, train_labels, val_data, val_cat, val_labels, is_continue=True):
        ...
        for step in range(1, num_steps + 1):
            ...
            if step % self.eval_frequency == 0 or step == num_steps:
                ...
                string, accuracy, f1, loss, void = self.evaluate(val_data, val_cat, val_labels, sess) # Ignore fifth argument
        ...
        return accuracies, losses, t_step, epoch_counter
...
class lancheb_gcn(base_model):
    ...
    def lanczos5(self, x, L, Fout, K):      # Implement Lanczos-Chebyshev hybrid filtering
        ...
        eigenval_zero = tf.nn.relu(eigenval)    # Ignore all eigenvalues that are negative
        eig_k = eigenval_zero[K,K]              # Take the K-th eigenvalue
        ...
        # Initialization of Chebyshev polynomials
        if K > 1:
            x1 = tf.matmul(L, x0)
            x = concat(x, x1)

        # Iteration of Chebyshev
        for k in range(2, K):
            x2 = 2 * tf.matmul(L, x1)  - x0 - (eig_k/tf.math.reduce_euclidean_norm(x))
            x = concat(x, x2)
            x0, x1 = x1, x2
        # K x N x M x Fin
